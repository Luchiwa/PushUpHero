...
export function Dashboard({ exerciseState, goalReps, sessionMode, timeGoal, onStop, onTimerEnd, elapsedTimeRef, onFlipCamera, facingMode, soundEnabled, onSoundToggle, level, levelProgressPct }: DashboardProps) {
    ...
    useEffect(() => {
        if (!isCalibrated) { setTimeout(() => setShowInvalidBanner(false), 0); return; }
        if (!isValidPosition) {
            invalidFramesRef.current++;
            if (invalidFramesRef.current >= 60) setTimeout(() => setShowInvalidBanner(true), 0);
        } else {
            invalidFramesRef.current = 0;
            setTimeout(() => setShowInvalidBanner(false), 0);
        }
    ...
    useEffect(() => {
        if (sessionMode === 'time') {
            ...
            setTimeout(() => setTimeRemaining(totalSeconds), 0);
