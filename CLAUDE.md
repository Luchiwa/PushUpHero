...
- **Coach phrases live in `coach.json`.** Categories are `coach.feedback.<RepFeedback>`, `coach.encouragement`, `coach.calibration.<ExerciseType>`, `coach.incomplete_rep.<ExerciseType>`. The anti-repeat rotation tracker keys by category, so granularity must be preserved — adding a new category requires both FR and EN entries with at least one phrase. The exercise registry no longer carries phrase arrays; new exercise types add their `coach:calibration.<type>` and `coach:incomplete_rep.<type>` entries to `coach.json`.
