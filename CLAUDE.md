...
- **Coach phrases live in `coach.json`.** Categories are `coach.feedback.<RepFeedback>`, `coach.encouragement`, `coach.calibration.<ExerciseType>`, `coach.incomplete_rep.<ExerciseType>`, `coach.set_complete`, `coach.rest_start`, `coach.rest_ending`, `coach.level_up`. The anti-repeat rotation tracker (`lastPicked` map in `coachEngine.ts`) keys by full i18n key, so granularity must be preserved — adding a new category requires both FR and EN entries with at least one phrase. The tracker is cleared on workout start (`useWorkoutExecution.handleStart` / `handleWorkoutStart` / `handleResumeWorkout`) — never per Dashboard mount, which would wipe per-rep rotation mid-workout in multi-exercise plans. The exercise registry no longer carries phrase arrays; new exercise types add their `coach:calibration.<type>` and `coach:incomplete_rep.<type>` entries to `coach.json`.
