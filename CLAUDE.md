...
- **StartScreen** lazy-loads modals/screens: `AuthModal`, `ProfileScreen`, `SocialScreen`, `SavedWorkoutsScreen`, `QuickSessionModal`, `StatsScreen`, `QuestsScreen`
...
### Internationalization (i18n) rules (enforced by reviewers)

The app ships in FR and EN (PUS-7). i18next is initialized in `src/i18n/index.ts` with nine namespaces — `common`, `start`, `workout`, `stats`, `quests`, `modals`, `coach`, `errors`, `dashboard` — split by UI surface for maintainability. The active locale comes from a custom `pushupHeroStorage` detector (delegating to `@infra/storage`) → `navigator.language` → fallback `en`, persisted to localStorage and (when signed in) mirrored to Firestore `profile.preferredLanguage`. These invariants keep new copy from drifting back into hardcoded English.

- **No user-facing string literals in components.** Always `t('namespace:key')`. The only tolerated exceptions: technical aria-labels in primitive components that don't already pull in `useTranslation`, and even then prefer `t('common:action.…')`. Brand names (e.g. `"Push-Up Hero"` in `AppLoader`) and universal symbols (grade letters S/A/B/C/D, exercise emojis) stay verbatim.
- **Namespacing.** One namespace per UI surface — don't add a "misc" bucket to `common`. New surface = new namespace declared in `src/i18n/index.ts`. `common` is for cross-cutting things only (action verbs, units, error fallbacks, reload prompt).
- **Granularity.** One key per logical sentence, not per word. Forbidden: concatenating `t('a') + ' ' + t('b')` — gender, word order, and punctuation differ across languages. Use `{{var}}` interpolation or a `<Trans>`-style prefix/strong/suffix split for inline `<strong>`. The InstallBanner iOS guide is the canonical inline-strong split.
- **Pluralization.** Always via i18next's `count`. Suffixes `_one` / `_other`. Example: `t('common:unit.rep', { count: 12 })`. Don't hand-roll `${n} rep${n>1?'s':''}`.
- **Keys at the boundary, `t()` at the consumer.** Definition layers (achievements, quests, records, XP bonuses, position guides, exercise registry, auth errors) carry `titleKey` / `descriptionKey` / `labelKey` / `*ParamsKey` fields, *never* English strings. The consumer (a component) calls `t(def.titleKey, def.titleParams)`. Helpers `getAchievementTitle/Description`, `getQuestTitle/Description`, `getRecordLabel`, `getExerciseLabelKey`, `getInvalidPositionMessageKey`, `getExerciseCategoryKey` exist for this — use them rather than inlining a `t(\`achievements:by_id.${id}.title\`)` call at every site.
- **Coach phrases live in `coach.json`.** Categories are `coach.feedback.<RepFeedback>`, `coach.encouragement`, `coach.calibration.<ExerciseType>`, `coach.incomplete_rep.<ExerciseType>`, `coach.set_complete`, `coach.rest_start`, `coach.rest_ending`, `coach.level_up`. The anti-repeat rotation tracker keys by full i18n key, so granularity must be preserved — adding a new category requires both FR and EN entries with at least one phrase. The tracker is cleared on workout start — never per Dashboard mount, which would wipe per-rep rotation mid-workout in multi-exercise plans. Milestone speech (`set_complete` / `rest_start` / `rest_ending` / `level_up`) bypasses the per-rep cooldown but Chrome's `speechSynthesis.cancel()` workaround means adjacent calls cancel each other — the wiring sites stagger them (set_complete only fires on workout end; level_up suppresses set_complete for 3 s). The exercise registry no longer carries phrase arrays; new exercise types add their `coach:calibration.<type>` and `coach:incomplete_rep.<type>` entries to `coach.json`.
- **Auth errors return keys.** `translateAuthError(err)` returns `'errors:auth.<code>'` (or a key carried in `err.message` for pre-keyed throws like `'errors:auth.username_taken'`). The caller does `t(translateAuthError(err))`. Same pattern applies to any future error-translation helper — return keys, never strings.
- **Locale-aware Intl via `@domain/format`.** `formatDate(date, opts?)`, `formatTime(date, opts?)`, `formatNumber(value, opts?)` read the locale from `i18next.language`. Use them for user-facing dates and numbers — never `toLocaleDateString*('en-US', …)` or bare `.toLocaleString()` in UI. The `'sv-SE'` YYYY-MM-DD trick used internally for date keys (sorting, storage keys) is technical formatting and stays.
- **Speech follows the language.** `speechEngine.speak(text, { lang })` accepts a BCP-47 override; otherwise derives from `mapToBcp47(i18next.language)`. Voice selection (`VOICE_PREFERENCES`) is per-language — adding a new locale means adding its preferred voices to that map. Never hardcode `'en-US'` on a `SpeechSynthesisUtterance.lang`.
- **Persistence + sync direction.** localStorage cache via the custom `pushupHeroStorage` detector (one of `STORAGE_KEYS`). When signed in, `profile.preferredLanguage` mirrors to Firestore via `useChangeLanguage` (client → cloud, on click). The reverse (cloud → client) is owned by `useSyncCloud`'s `onUserDoc` callback at sign-in. Don't write a parallel sync — one direction per hook.
- **`<html lang>` is the listener's job.** A single `i18n.on('languageChanged', …)` listener in `src/i18n/index.ts` updates `document.documentElement.lang`. Don't add another anywhere — the listener already runs on every change, including the cloud-driven one.
- **Adding a language.** Create `src/i18n/locales/<lang>/<each-namespace>.json`, register it in `supportedLngs` + the resources block in `src/i18n/index.ts`, add a `mapToBcp47` branch + voice preferences in `speechEngine.ts`, extend `SUPPORTED_UI_LANGS` in `useSyncCloud` and `useChangeLanguage`'s `SupportedLanguage` union, add the option to `LanguagePreferenceSection`. No other code changes if every namespace is filled.
